<template>
  <v-card class="mx-auto" elevation="2" rounded="lg">
    ...
    >
      ...
      <v-toolbar-title
        ...
        {{
          todoTree.reduce(
            (sum, category) =>
              sum + (category.children ? category.children.length : 0),
            0
          )
        }}</v-toolbar-title
      >
      <v-btn @click="fetchTodos"><v-icon>mdi-refresh</v-icon></v-btn>
    ...
    <vue-markdown
      v-if="showTodos && summary"
      :source="summary"
      class="prose max-w-none markdown-preview w-full p-4 !text-white"
    />
...
async function fetchGroqApiKey() {
  ...
  const groqApiKey = notes.value[0]?.children[0]?.text;
  if (groqApiKey) {
    groq = new Groq({
      apiKey: groqApiKey,
...
async function categorizeTodos(todos) {
  ...
  todoTree.value = Object.entries(categorizedTodos).map(
    ([category, items]) => ({
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a consise and accurate AI assistant that analyzes todo lists to help the user become more efficient.",
        },
        {
          role: "user",
...
async function categorizeTodosWithGroq(todos) {
  ...
  try {
    ...
    const categorizedTodos = JSON.parse(
      chatCompletion.choices[0]?.message?.content ||
        `{['text':'Error: '+${JSON.stringify(chatCompletion)}]`
    );
